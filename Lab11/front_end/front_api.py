...
class Controller:
    def __init__(self):
        ...
        self.__restaurant_owner_account_list = []
        self.__restaurant_list = []
        self.__order_list = []
        self.__central_money = None
    ...
    def central_money(self, system_balance):
        self.__central_money = system_balance 
        
    def add_customer_account_list(self, new_customer):
        self.__customer_account_list.append(new_customer)
        
    def add_rider_account_list(self, new_rider):
        self.__rider_account_list.append(new_rider)
        
    def add_restaurant_owner_account_list(self, new_restaurant_owner):
        self.__restaurant_owner_account_list.append(new_restaurant_owner)
        
    def add_restaurant_list(self, new_restaurant):
        self.__restaurant_list.append(new_restaurant)
        
    def add_order_list(self, new_order):
    ...
    def search_restaurant_owner_by_id(self, restaurant_owner_id):
        for restaurant_owner in self.__restaurant_owner_account_list:
    ...
    def add_order_into_cart(self, customer_id, order_id):
        ...
        return f"{customer.account_id} added {order.order_id} into cart"
...
rider01.profile = profile_rider02
rider01.pocket = pocket_rider01
rider01.pocket = pocket_rider02
...
def submit_show_order_cart():
    # Get customer ID from entry widget
    customer_id = str(show_order_cart_customer_id_entry.get())
    
    # Retrieve orders for the given customer ID
    result = controller.show_customer_order_cart(customer_id)
    
    # Clear any previous order containers and reset the result label
    for widget in show_order_cart_container.winfo_children():
        widget.destroy()
    show_order_cart_result_label.config(text="")
    
    # Check if orders are found for the given customer ID
    if result:
        # Display customer ID label
        customer_id_label = ttk.Label(show_order_cart_container, text=f"Customer ID: {customer_id}")
        customer_id_label.grid(row=0, column=0, columnspan=2, padx=5, pady=5) 
        
        # Display each order in separate containers
        for index, order in enumerate(result):
            order_container = ttk.Frame(show_order_cart_container, borderwidth=2, relief="solid")
            order_container.grid(row=index+1, column=0, columnspan=2, padx=5, pady=5)

            order_id_label = ttk.Label(order_container, text=f"Order ID: {order['order_id']}")
            order_id_label.grid(row=0, column=0, columnspan=2, padx=5, pady=5)
    else:
        # If no orders are found, display a message
        show_order_cart_result_label.config(text="No orders found")

def submit_add_order():
    customer_id = str(add_order_customer_id_entry.get())
    order_id = str(add_order_order_id_entry.get())
    result = controller.add_order_into_cart(customer_id, order_id)
    add_order_result_label.config(text=result)
    
def submit_remove_order():
    customer_id = str(remove_order_customer_id_entry.get())
    order_id = str(remove_order_order_id_entry.get())
    result = controller.remove_order_from_cart(customer_id, order_id)
    remove_order_result_label.config(text=result)
    
def submit_confirm_order():
    customer_id = str(confirm_order_customer_id_entry.get())
    order_id = str(confirm_order_order_id_entry.get())
    result = controller.confirm_customer_order(customer_id, order_id)
    confirm_order_result_label.config(text=result)

root = tk.Tk()
root.title("Quick-Meal")
root.geometry("900x600")

style = ttk.Style()
style.theme_use('superhero')

# container_of_show_customer_order_cart

show_order_cart_container = ttk.Frame(root, borderwidth=2, relief="solid")
show_order_cart_container.grid(row=0, column=0, padx=10, pady=10)

show_order_cart_text = ttk.Label(show_order_cart_container, text='Show Customer Order Cart')
show_order_cart_text.grid(row=0, column=0, columnspan=2, padx=5, pady=5)

show_order_cart_customer_id_label = ttk.Label(show_order_cart_container, text="Customer ID:")
show_order_cart_customer_id_label.grid(row=1, column=0, padx=5, pady=5)

show_order_cart_customer_id_entry = ttk.Entry(show_order_cart_container)
show_order_cart_customer_id_entry.grid(row=1, column=1, padx=5, pady=5)

show_order_cart_submit_button = ttk.Button(show_order_cart_container, text="Submit", command=submit_show_order_cart)
show_order_cart_submit_button.grid(row=2, column=0, columnspan=2, padx=5, pady=5)

show_order_cart_result_label = ttk.Label(show_order_cart_container, text="")
show_order_cart_result_label.grid(row=3, column=0, columnspan=2, padx=5, pady=5)

# reset_button = ttk.Button(root, text="Reset", command=reset_show_order_cart)
# reset_button.grid(row=0, column=1, columnspan=2, padx=5, pady=5)

# container_of_add_order

add_order_container = ttk.Frame(root, borderwidth=2, relief="solid")
add_order_container.grid(row=1, column=0, padx=10, pady=10)

add_order_text = ttk.Label(add_order_container, text='Add Order')
add_order_text.grid(row=0, column=0, columnspan=2, padx=5, pady=5)

add_order_customer_id_label = ttk.Label(add_order_container, text="Customer ID:")
add_order_customer_id_label.grid(row=1, column=0, padx=5, pady=5)

add_order_customer_id_entry = ttk.Entry(add_order_container)
add_order_customer_id_entry.grid(row=1, column=1, padx=5, pady=5)

add_order_order_id_label = ttk.Label(add_order_container, text="Order ID:")
add_order_order_id_label.grid(row=2, column=0, padx=5, pady=5)

add_order_order_id_entry = ttk.Entry(add_order_container)
add_order_order_id_entry.grid(row=2, column=1, padx=5, pady=5)

add_order_submit_button = ttk.Button(add_order_container, text="Submit Order", command=submit_add_order)
add_order_submit_button.grid(row=3, column=0, columnspan=2, padx=5, pady=5)

add_order_result_label = ttk.Label(add_order_container, text="")
add_order_result_label.grid(row=4, column=0, columnspan=2, padx=5, pady=5)

# container_of_remove_order

remove_order_container = ttk.Frame(root, borderwidth=2, relief="solid")
remove_order_container.grid(row=1, column=1, padx=10, pady=10)

remove_order_text = ttk.Label(remove_order_container, text='Remove Order')
remove_order_text.grid(row=0, column=0, columnspan=2, padx=5, pady=5)

remove_order_customer_id_label = ttk.Label(remove_order_container, text="Customer ID:")
remove_order_customer_id_label.grid(row=1, column=0, padx=5, pady=5)

remove_order_customer_id_entry = ttk.Entry(remove_order_container)
remove_order_customer_id_entry.grid(row=1, column=1, padx=5, pady=5)

remove_order_order_id_label = ttk.Label(remove_order_container, text="Order ID:")
remove_order_order_id_label.grid(row=2, column=0, padx=5, pady=5)

remove_order_order_id_entry = ttk.Entry(remove_order_container)
remove_order_order_id_entry.grid(row=2, column=1, padx=5, pady=5)

remove_order_submit_button = ttk.Button(remove_order_container, text="Submit Order", command=submit_remove_order)
remove_order_submit_button.grid(row=3, column=0, columnspan=2, padx=5, pady=5)

remove_order_result_label = ttk.Label(remove_order_container, text="")
remove_order_result_label.grid(row=4, column=0, columnspan=2, padx=5, pady=5)

# container_of_confirm_order

confirm_order_container = ttk.Frame(root, borderwidth=2, relief="solid")
confirm_order_container.grid(row=2, column=0, columnspan=2, padx=10, pady=10)

confirm_order_text = ttk.Label(confirm_order_container, text='Confirm Order')
confirm_order_text.grid(row=0, column=0, columnspan=2, padx=5, pady=5)

confirm_order_customer_id_label = ttk.Label(confirm_order_container, text="Customer ID:")
confirm_order_customer_id_label.grid(row=1, column=0, padx=5, pady=5)

confirm_order_customer_id_entry = ttk.Entry(confirm_order_container)
confirm_order_customer_id_entry.grid(row=1, column=1, padx=5, pady=5)

confirm_order_order_id_label = ttk.Label(confirm_order_container, text="Order ID:")
confirm_order_order_id_label.grid(row=2, column=0, padx=5, pady=5)

confirm_order_order_id_entry = ttk.Entry(confirm_order_container)
confirm_order_order_id_entry.grid(row=2, column=1, padx=5, pady=5)

confirm_order_submit_button = ttk.Button(confirm_order_container, text="Submit Order", command=submit_confirm_order)
confirm_order_submit_button.grid(row=3, column=0, columnspan=2, padx=5, pady=5)

confirm_order_result_label = ttk.Label(confirm_order_container, text="")
confirm_order_result_label.grid(row=4, column=0, columnspan=2, padx=5, pady=5)
